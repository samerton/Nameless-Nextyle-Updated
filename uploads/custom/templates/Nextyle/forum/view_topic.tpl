...
<div class="container" style="padding-top: 2rem;">
  ...
  <span class="pull-right">
	{if isset($CAN_REPLY)}
	  <div class="btn-group">
		<a{if isset($LOCKED) && !isset($CAN_MODERATE)} disabled="disabled"{else} href="#reply_section"{/if} style="vertical-align:baseline;" class="btn btn-{if isset($LOCKED) && !isset($CAN_MODERATE)}warning disabled{else}{$NEXTYLE_COLOR}{/if}">{if isset($LOCKED) && !isset($CAN_MODERATE)}<i class="fa fa-lock" aria-hidden="true"></i> {/if}{$NEW_REPLY}</a>
	  </div>
	{/if}
    {if isset($CAN_MODERATE)}
    <div class="btn-group">
	  <button type="button" style="vertical-align:baseline;" class="btn btn-{$NEXTYLE_COLOR} dropdown-toggle" data-toggle="dropdown">{$MOD_ACTIONS} <span class="caret"></span></button>
    ...
    <div class="btn-group">
	  <button type="button" style="vertical-align:baseline;" class="btn btn-{$NEXTYLE_COLOR} dropdown-toggle" data-toggle="dropdown">{$SHARE} <span class="caret"></span></button>
	  <ul class="dropdown-menu" role="menu">
    ...
    <div class="panel panel-{$NEXTYLE_COLOR}">
	  ...
	  <div class="panel-body" id="post-{$reply.id}">
	    <div class="row">
		  <div class="col-md-3">
		    <center>
			  <img class="img-rounded" style="max-width:100px; max-height:100px;" src="{$reply.avatar}" />
			  <br /><br />
  ...
  <div id="reply_section">
    ...
    <form action="" method="post">
	  ...
	  <br />
	  <input type="hidden" name="token" value="{$TOKEN}">
	  <button type="submit" class="btn btn-{$NEXTYLE_COLOR}">{$SUBMIT}</button>
	  <button type="button" class="btn btn-{$NEXTYLE_COLOR}" id="quoteButton" onclick="insertQuotes();">Insert Quotes</button>
    </form>
  </div>
  {/if}
