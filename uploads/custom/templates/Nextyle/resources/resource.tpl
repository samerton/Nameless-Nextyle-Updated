  ...
  <div class="card">
    <div class="card-block">
	  ...
	  <span class="pull-right">
	    {if isset($CAN_UPDATE)}
		  <a href="{$UPDATE_LINK}" class="btn btn-{$NEXTYLE_COLOR}">{$UPDATE}</a>
		{/if}
		<a href="{$BACK_LINK}" class="btn btn-{$NEXTYLE_COLOR}">{$RESOURCE_INDEX}</a>
	  </span>
	  ...
	  <div class="row">
	    <div class="col-md-9">
		  ...
		  <a href="{$DOWNLOAD_URL}" class="btn btn-{$NEXTYLE_COLOR}" target="_blank">{$DOWNLOAD}</a>
		  
		  <span class="pull-right">
			{if isset($CAN_EDIT)}<a href="{$EDIT_LINK}" class="btn btn-{$NEXTYLE_COLOR}">{$EDIT}</a>{/if}
			{if isset($MODERATION) && count($MODERATION)}
			  <div class="btn-group dropup">
				<button type="button" class="btn btn-{$NEXTYLE_COLOR} dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{$MODERATION_TEXT}</button>
			...
			{/if}
		    <a href="{$OTHER_RELEASES_LINK}" class="btn btn-{$NEXTYLE_COLOR}">{$OTHER_RELEASES}</a>
		  </span>
