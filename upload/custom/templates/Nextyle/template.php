...
class Nextyle_Template extends TemplateBase {
	...
	public function __construct($smarty, $language, $user, $pages){
		...
		$this->addCSSFiles(array(
			(defined('CONFIG_PATH') ? CONFIG_PATH : '') . '/custom/templates/Nextyle/css/bootstrap.min.css' => array(),
			...
			(defined('CONFIG_PATH') ? CONFIG_PATH : '') . '/custom/templates/Nextyle/css/custom.css' => array()
		));

		// Add any JS files here
		$this->addJSFiles(array(
			(defined('CONFIG_PATH') ? CONFIG_PATH : '') . '/custom/templates/Nextyle/js/jquery.min.js' => array(),
			(defined('CONFIG_PATH') ? CONFIG_PATH : '') . '/custom/templates/Nextyle/js/popper.min.js' => array(),
			(defined('CONFIG_PATH') ? CONFIG_PATH : '') . '/custom/templates/Nextyle/js/tether.min.js' => array(),
			(defined('CONFIG_PATH') ? CONFIG_PATH : '') . '/custom/templates/Nextyle/js/bootstrap.min.js' => array(),
	...
	public function onPageLoad(){
		...
		if(defined('FRONT_END')){
			...
			$this->addCSSStyle('.nextyle-headtop {background: url(\'' . $nextyle_bg . '\') no-repeat center top;}');
		}
										    ...
										    if(serverPlayerList == 1){
										        if(data.player_list.length > 0){
										            ...
										            for(var i = 0; i < data.player_list.length; i++){
										                html += "<a href=\"' . URL::build('/profile/') . '" + data.player_list[i].name + "\"><img style=\"margin-bottom:3px;max-width:32px;max-height:32px;\" data-toggle=\"tooltip\" title=\"" + data.player_list[i].name + "\" src=\"" + avatarSource.replace("{x}", data.player_list[i].id).replace("{y}", 64) + "\" class=\"img-rounded\" alt=\"" + data.player_list[i].name + "\"></a> ";
										            }
										            html += "</p>";
...
$template = new Nextyle_Template($smarty, $language, $user, $pages);