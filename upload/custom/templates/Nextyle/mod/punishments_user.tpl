            ...
            <div class="card">
                <div class="card-block">
                    ...
                    <span class="pull-right">
                        <a href="{$BACK_LINK}" class="btn btn-{$NEXTYLE_COLOR}">{$BACK}</a>
                    </span>
                    <hr />
                    <h4>{$VIEWING_USER}</h4>
                    {if isset($ERROR)}
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4" style="text-align: center">
                            <a href="#" data-toggle="modal" data-target="#warnModal" class="btn btn-{$NEXTYLE_COLOR}">{$WARN}</a>
                        </div>
                        {/if}
                        {if isset($BAN)}
                        <div class="col-md-4" style="text-align: center">
                            <a href="#" data-toggle="modal" data-target="#banModal" class="btn btn-red">{$BAN}</a>
                        </div>
                        {/if}
                        {if isset($BAN_IP)}
                        <div class="col-md-4" style="text-align: center">
                            <a href="#" data-toggle="modal" data-target="#banIPModal" class="btn btn-red">{$BAN_IP}</a>
                        </div>
                        {/if}
                    </div>
                    <hr />
                            ...
                            <div class="card">
                                <div class="card-header">
                                    {if $punishment.type_numeric == 1}
                                        <span class="tag tag-danger">{$punishment.type}</span>
                                    {elseif $punishment.type_numeric == 2}
                                        <span class="tag tag-warning">{$punishment.type}</span>
                                    {elseif $punishment.type_numeric == 3}
                                        <span class="tag tag-danger">{$punishment.type}</span>
                                    {/if}
                                    {if $punishment.revoked == 1}
                                        <span class="tag tag-info">{$REVOKED}</span>
                                    {/if}
                                    {if $punishment.acknowledged == 1}
                                        <span class="tag tag-success">{$ACKNOWLEDGED}</span>
                                    {/if}
